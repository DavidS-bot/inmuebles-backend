...
def delete_property(pid: int, session: Session = Depends(get_session), user=Depends(get_current_user)):
    from ..models import Rule, Movement, FinancialMovement, RentalContract, MortgageDetails, ClassificationRule
    ...
    try:
        ...
        classification_rules = session.exec(
            select(ClassificationRule).where(ClassificationRule.property_id == pid)
        ).all()
        for rule in classification_rules:
            session.delete(rule)
        
        # Delete financial movements
        financial_movements = session.exec(
            select(FinancialMovement).where(FinancialMovement.property_id == pid)
        ).all()
        for movement in financial_movements:
            session.delete(movement)
        
        # Delete rental contracts (and related documents)
        rental_contracts = session.exec(
            select(RentalContract).where(RentalContract.property_id == pid)
        ).all()
        for contract in rental_contracts:
            session.delete(contract)
        
        # Delete mortgage details
        mortgage_details = session.exec(
            select(MortgageDetails).where(MortgageDetails.property_id == pid)
        ).all()
        ...
        # Delete rules
        rules = session.exec(
            select(Rule).where(Rule.property_id == pid)
        ).all()
        for rule in rules:
            session.delete(rule)
        ...
        movements = session.exec(
            select(Movement).where(Movement.property_id == pid)
        ).all()
        ...
        return {"message": "Propiedad eliminada correctamente junto con todos sus datos relacionados"}
        
    except Exception as e:
        session.rollback()
        raise HTTPException(500, f"Error al eliminar la propiedad: {str(e)}")
